...
namespace svo
{
	...
	const std::unordered_map<svo::Stage, std::string, EnumClassHash> kStageName{{Stage::kPaused, "Paused"}, {Stage::kInitializing, "Initializing"}, {Stage::kTracking, "Tracking"}, {Stage::kRelocalization, "Reloc"}};
	...
	FrameHandlerBase::FrameHandlerBase(const BaseOptions &base_options, const ReprojectorOptions &reprojector_options,
									   ...
									   const FeatureTrackerOptions &tracker_options, const CameraBundle::Ptr &cameras) : options_(base_options), cams_(cameras), stage_(Stage::kPaused), set_reset_(false), set_start_(false), map_(new Map), acc_frame_timings_(10), acc_num_obs_(10), num_obs_last_(0), tracking_quality_(TrackingQuality::kInsufficient), relocalization_n_trials_(0)
	{
		// sanity checks
		...
		if (user_schur_vins)
		{
			...
			if (cams_->getCamera(0).getType() == vk::cameras::CameraGeometryBase::Type::kPinhole)
			{ // init focal length
	...
	//------------------------------------------------------------------------------
	/**
	 * @brief 添加图像束到SVO系统进行处理
	 * @param imgs 输入图像数组，单目系统包含1张图像，双目系统包含2张图像
	 * @param timestamp 图像时间戳（纳秒）
	 * @return 处理是否成功
	 *
	 * 该函数是SVO视觉前端的核心入口之一，负责：
	 * 1. 验证图像时间戳的有效性
	 * 2. 将输入的原始图像转换为SVO内部的Frame对象
	 * 3. 创建图像束(FrameBundle)对象
	 * 4. 调用addFrameBundle进行后续处理
	 */
	bool FrameHandlerBase::addImageBundle(const std::vector<cv::Mat> &imgs, const uint64_t timestamp)
	{
		// 检查图像时间戳是否有效，确保新图像的时间戳晚于上一帧
		if (last_frames_)
		{
			// 如果新图像时间戳早于或等于最后一帧时间戳，则丢弃该图像
			if (last_frames_->getMinTimestampNanoseconds() >= static_cast<int64_t>(timestamp))
			{
				// 输出详细日志信息，包含丢弃原因和上一帧的ID
		...
		// 性能监控：开始计时，用于统计金字塔创建的时间
		if (options_.trace_statistics)
		{
			SVO_START_TIMER("pyramid_creation");
		}

		// 检查输入图像的数量是否与系统配置的相机数量一致
		// 这是一个断言，确保输入数据的正确性，不一致会导致程序终止
		...
		for (size_t i = 0; i < imgs.size(); ++i)
		{
			...
			// 设置相机到IMU的变换矩阵，建立相机坐标系与IMU坐标系的关系
			frames.back()->set_T_cam_imu(cams_->get_T_C_B(i));

			// 设置该帧在多相机系统中的索引(0表示左目，1表示右目等)
			frames.back()->setNFrameIndex(i);
		}

		// 创建FrameBundle对象，将所有Frame对象打包在一起
		// 在双目系统中包含左右目两个Frame，单目系统中只包含一个Frame
		FrameBundlePtr frame_bundle(new FrameBundle(frames));

		// 性能监控：结束计时，完成金字塔创建时间的统计
		if (options_.trace_statistics)
		{
			SVO_STOP_TIMER("pyramid_creation");
		}

		// 调用addFrameBundle函数进行实际的图像束处理
		// 这是实际处理流程的入口点
	...
	bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr &frame_bundle)
	{
		...
		// 如果系统处于暂停状态，并且接收到启动信号，则重置系统并设置初始位姿
		if (set_start_)
		{
		...
		double opt_dist_first_two_kfs = -1; // 存储后端优化后前两个关键帧之间的距离，初始化为-1
		bool backend_scale_stable = false;
		if (bundle_adjustment_) // 检查是否存在Bundle Adjustment后端优化器
		{
			if (map_->size() > 2)
			{
				// 如果地图中有超过2个关键帧，计算优化前前两个关键帧之间的距离。这个距离将作为尺度变化的基准
				svo_dist_first_two_kfs = distanceFirstTwoKeyframes(*map_);
			}

			// 检查是否需要重新初始化，如果需要则恢复之前保存的速度偏置、位姿和时间戳状态
			...
			// 如果后端有帧数据且有运动先验，获取最新的速度偏置和位姿信息，并将重新初始化标志设为false
			...
			/* 重新计算优化后前两个关键帧的距离
			计算尺度变化：scale_change = (优化后距离 / 优化前距离) - 1.0
			如果尺度变化的绝对值小于阈值backend_scale_stable_thresh，则认为尺度稳定
			相应地设置第一个关键帧的稳定性标志和backend_scale_stable变量
			如果尺度不稳定，输出警告信息 */
			if (map_->size() > 2)
			{
		...
		if (schur_vins_ && imu_handler_)
		{
			...
			const double curr_frame_bundle_stamp = frame_bundle->getMinTimestampSeconds(); // 获取当前FrameBundle中最小时间戳的图像时间
			bool imu_ready = imu_handler_->waitTill(curr_frame_bundle_stamp);			   // 等IMU数据到达当前图像时间，仅等待5ms
			// 取出两帧之间的IMU数据
			if (imu_ready && imu_handler_->getMeasurementsContainingEdges(curr_frame_bundle_stamp, imu_measurements, true))
			{
