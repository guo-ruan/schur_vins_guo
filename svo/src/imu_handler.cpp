...
namespace svo
{
    ...
    /**
     * @brief 获取与给定时间戳最接近的IMU测量数据
     *
     * 该函数在已存储的IMU测量值中查找与给定时间戳最接近的测量数据，
     * 考虑了IMU和相机之间的时间延迟，并检查找到的测量值是否在允许的时间差范围内。
     *
     * @param timestamp 目标时间戳（秒），通常是相机图像的时间戳
     * @param measurement [输出参数] 最接近目标时间戳的IMU测量数据
     * @return bool 操作是否成功，true表示找到有效测量值，false表示失败
     */
    bool ImuHandler::getClosestMeasurement(
        const double timestamp,            // 输入：目标时间戳（秒）
        ImuMeasurement &measurement) const // 输出：最接近的IMU测量数据
    {
        // 加锁以确保对测量数据的线程安全访问
        ulock_t lock(measurements_mut_);
        ...
        const ImuMeasurement &m)
    {
        ...
        if (options_.temporal_stationary_check) // 检查是否处于静止状态
        {
            temporal_imu_window_.push_front(m); // TODO？：静止状态下，放入该队列，可能需要进行初始化
        }
        return true;
    }
    ...
    // 选择重力方向作为Z轴方向
    // 计算并返回 IMU 坐标系相对于世界坐标系的初始姿态（旋转四元数）。该姿态通过测量的重力方向（线性加速度）来确定
