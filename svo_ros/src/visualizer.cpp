  ...
  , vis_scale_(vk::param<double>(pnh_, "publish_marker_scale", 1.2))
{
  ...
  if (!boost::filesystem::exists(logs_dir)) {
    LOG(INFO) << "Log directory does not exist, creating: " << logs_dir;
    bool success = boost::filesystem::create_directories(logs_dir);
    CHECK(success) << "Failed to create log directory: " << logs_dir;
  }
